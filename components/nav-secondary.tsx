...
export function NavSecondary({
  ...
  return (
    <SidebarGroup>
      ...
      <SidebarMenu>
        {items.map((item) => (
          <SidebarMenuItem key={item.title} className="group relative my-1 overflow-hidden">
            <SidebarMenuButton
              tooltip={item.title}
              onClick={() => handleItemClick(item.title, item.url)} // Pass the URL to handleItemClick
